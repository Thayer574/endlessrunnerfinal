...
class TriSplitBullet(Bullet):
    ...
    def draw(self, surface):
        ...
        pygame.draw.circle(glow_surf, (*self.color, 150), (int(current_radius * 2), int(current_radius * 2)), int(current_radius * 2.5))
        pygame.draw.circle(glow_surf, (*self.color, 100), (int(current_radius * 2), int(current_radius * 2)), int(current_radius * 3))
...
class HomingOrb(Bullet):
    def __init__(self, pos, owner, damage, target_enemy, color):
        super().__init__(pos, pygame.math.Vector2(0,0), owner, damage, color=color, radius=6)
        self.target = target_enemy
        self.speed = PLAYER_BULLET_SPEED * 0.8
        self.turn_speed = 8  # Increased turn speed for better homing
        self.lifetime = 3.0
        self.spiral_angle = 0

    def update(self, dt, enemies):
        self.spiral_angle += dt * 10
    ...
    def draw(self, surface):
        # Draw spiral trail
        draw_spinning_rings(surface, self.pos, self.color, 15, self.spiral_angle, num_rings=2)
