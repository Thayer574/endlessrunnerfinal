...
class BossProjectile(Bullet):
    def __init__(self, pos, vel, owner, damage, color, radius, boss_type="red"):
    ...
    def update(self, dt):
        super().update(dt)
        self.angle += dt * 5
        self.pulse = (math.sin(pygame.time.get_ticks() * 0.01) + 1) * 0.5
    ...
    def draw(self, surface):
        ...
        if self.boss_type == "red":
            # Red Core: Jagged, unstable energy orb
            glow_surf = pygame.Surface((self.radius * 6, self.radius * 6), pygame.SRCALPHA)
            for i in range(3):
                r = self.radius * (2 + i * 0.5) * (1 + self.pulse * 0.2)
                pygame.draw.circle(glow_surf, (255, 0, 0, 40 - i * 10), (self.radius * 3, self.radius * 3), r)
            surface.blit(glow_surf, (int(self.pos.x - self.radius * 3), int(self.pos.y - self.radius * 3)))
            
            # Core circle
            pygame.draw.circle(surface, (255, 50, 50), (int(self.pos.x), int(self.pos.y)), self.radius)
            # Inner white core
            pygame.draw.circle(surface, (255, 255, 255), (int(self.pos.x), int(self.pos.y)), self.radius // 2)

        elif self.boss_type == "orange":
            # Orange Juggernaut: Heavy, spinning gear/meteor
            s = pygame.Surface((self.radius * 2, self.radius * 2), pygame.SRCALPHA)
            pygame.draw.circle(s, (255, 140, 0), (self.radius, self.radius), self.radius)
            # Add "teeth" or spikes
            for i in range(4):
                a = self.angle + (i * math.pi / 2)
                px = self.radius + math.cos(a) * self.radius * 1.2
                py = self.radius + math.sin(a) * self.radius * 1.2
                pygame.draw.line(s, (255, 69, 0), (self.radius, self.radius), (px, py), 4)
            surface.blit(s, (int(self.pos.x - self.radius), int(self.pos.y - self.radius)))

        elif self.boss_type == "yellow":
            # Yellow Eye: Eye-like projectile with trailing iris
            pygame.draw.circle(surface, (255, 255, 200), (int(self.pos.x), int(self.pos.y)), self.radius)
            pygame.draw.circle(surface, (255, 255, 0), (int(self.pos.x), int(self.pos.y)), self.radius * 0.6)
            # Pupil
            pygame.draw.circle(surface, (0, 0, 0), (int(self.pos.x), int(self.pos.y)), self.radius * 0.3)

        elif self.boss_type == "final":
            # Ancient One: Void/Dark matter orb
            glow_surf = pygame.Surface((self.radius * 8, self.radius * 8), pygame.SRCALPHA)
            pygame.draw.circle(glow_surf, (100, 0, 255, 60), (self.radius * 4, self.radius * 4), self.radius * 3 * (1 + self.pulse * 0.3))
            surface.blit(glow_surf, (int(self.pos.x - self.radius * 4), int(self.pos.y - self.radius * 4)))
            
            pygame.draw.circle(surface, (20, 20, 20), (int(self.pos.x), int(self.pos.y)), self.radius)
            pygame.draw.circle(surface, (150, 0, 255), (int(self.pos.x), int(self.pos.y)), self.radius, 2)
...
class BossBase(EnemyBase):
    ...
    def draw(self, surface):
        ...
        # Base boss body with glow
        glow_radius = self.radius * (1.1 + pulse * 0.1)
        s = pygame.Surface((glow_radius * 2.5, glow_radius * 2.5), pygame.SRCALPHA)
        pygame.draw.circle(s, (*self.color, 100), (glow_radius * 1.25, glow_radius * 1.25), glow_radius)
        surface.blit(s, (int(self.pos.x - glow_radius * 1.25), int(self.pos.y - glow_radius * 1.25)))
        ...
        if getattr(self, 'flash_timer', 0) > 0:
            pygame.draw.circle(surface, self.flash_color, (int(self.pos.x), int(self.pos.y)), self.radius)
        ...
        if self.state == "normal":
            ...
            if int(current_time * 15) % 2 == 0:
                for i in range(3):
                    ...
                    p = BossProjectile(self.pos, dir_vec * 250, owner="enemy", damage=15 * getattr(self, "damage_mult", 1.0) *
                               self.damage_mult, color=(255, 50, 50), radius=8, boss_type="red")
            ...
            if self.state_timer <= 0:
                ...
                for speed_mult in [1.0, 1.5, 2.0]:
                    for angle in range(0, 360, 15):
                        rad = math.radians(angle)
                        dir_vec = pygame.math.Vector2(math.cos(rad), math.sin(rad))
                        p = BossProjectile(self.pos, dir_vec * (200 * speed_mult), owner="enemy", damage=20 *
                                   self.damage_mult, color=(255, 100, 100), radius=int(10 + (speed_mult * 2)), boss_type="red")
                        bullets_group.append(p)
                self.color = (180, 0, 0)
                self.state = "idle"
            ...
            if self.state_timer > 0:
                ...
                if int(current_time * 20) % 2 == 0:
                    # Aimed shot
                    aim_dir = normalize_safe(player.pos - self.pos)
                    p1 = BossProjectile(self.pos, aim_dir * 500, owner="enemy", damage=12 *
                                self.damage_mult, color=(255, 0, 0), radius=10, boss_type="red")
                    ...
                    rad = math.radians(angle)
                    dir_vec = pygame.math.Vector2(math.cos(rad), math.sin(rad))
                    p2 = BossProjectile(self.pos, dir_vec * random.uniform(200, 600), owner="enemy",
                                damage=8 * self.damage_mult, color=(255, 150, 0), radius=8, boss_type="red")
        ...
        if self.state == "idle":
            ...
            if self.state_timer > 0:
                ...
                if int(current_time * 30) % 2 == 0:
                    p = BossProjectile(self.pos, pygame.math.Vector2(0, 0), owner="enemy",
                               damage=10 * self.damage_mult, color=(255, 69, 0), radius=20, boss_type="orange")
                    p.lifetime = 1.0
                    bullets_group.append(p)
            else:
            ...
            if int(current_time * 10) % 3 == 0:
                for angle in range(0, 360, 30):
                    ...
                    dir_vec = pygame.math.Vector2(math.cos(rad), math.sin(rad))
                    p = BossProjectile(self.pos, dir_vec * 300, owner="enemy", damage=25 *
                               self.damage_mult, color=(255, 140, 0), radius=15, boss_type="orange")
                    bullets_group.append(p)
            if self.state_timer <= 0:
                self.state = "idle"
            ...
            if int(current_time * 15) % 2 == 0:
                ...
                p = BossProjectile(pygame.math.Vector2(spawn_x, 0), pygame.math.Vector2(0, 600),
                           owner="enemy", damage=35 * self.damage_mult, color=(255, 50, 0), radius=25, boss_type="orange")
                bullets_group.append(p)
            ...
            if self.state_timer > 0.5:
                ...
                for angle in range(0, 360, 10):
                    rad = math.radians(angle)
                    dir_vec = pygame.math.Vector2(math.cos(rad), math.sin(rad))
                    p = BossProjectile(self.pos, dir_vec * 400, owner="enemy", damage=30 *
                               self.damage_mult, color=(255, 69, 0), radius=20, boss_type="orange")
                    bullets_group.append(p)
        ...
        if self.state == "normal":
            ...
            if self.state_timer > 0:
                ...
                for i in range(5):  # Fire a burst
                    p = BossProjectile(self.pos, aim_dir * (1000 + i * 100), owner="enemy",
                               damage=30 * self.damage_mult, color=(255, 255, 0), radius=15, boss_type="yellow")
                    bullets_group.append(p)
            ...
            if int(current_time * 12) % 2 == 0:
                ...
                for angle in range(0, 360, 30):
                    ...
                    dir_vec = pygame.math.Vector2(math.cos(rad), math.sin(rad))
                    p = BossProjectile(self.pos, dir_vec * 350, owner="enemy", damage=20 *
                               self.damage_mult, color=(255, 255, 150), radius=12, boss_type="yellow")
                    bullets_group.append(p)
            if self.state_timer <= 0:
                self.state = "idle"
                self.attack_timer = 3.0
            ...
            if int(current_time * 15) % 3 == 0:
                ...
                for spread in range(-20, 21, 10):
                    ...
                    dir_vec = pygame.math.Vector2(math.cos(rad), -math.sin(rad))
                    p = BossProjectile(self.pos, dir_vec * 500, owner="enemy", damage=15 * getattr(self, "damage_mult", 1.0), color=(255, 255, 255), radius=14, boss_type="yellow")
                    bullets_group.append(p)
            if self.state_timer <= 0:
                self.state = "idle"
                self.attack_timer = 4.0
            ...
            if int(current_time * 25) % 2 == 0:
                ...
                rad = math.radians(aim_dir.angle_to(pygame.math.Vector2(1, 0)) + spread)
                dir_vec = pygame.math.Vector2(math.cos(rad), -math.sin(rad))
                p = BossProjectile(self.pos, dir_vec * 250, owner="enemy", damage=8, color=(255, 255, 0), radius=8, boss_type="yellow")
                bullets_group.append(p)
            if self.state_timer <= 0:
                self.state = "idle"
                self.attack_timer = 2.0
...
class AncientOneBoss(BossBase):
    def __init__(self, pos, wave):
        ...
        # Scaled up significantly: radius from 120 to 300
        super().__init__(pos, color=(20, 20, 20), radius=300, hp=hp, score_value=10000)
    ...
    def create_mini_boss(self, b_type, pos):
        ...
        m.max_hp /= 1.5
        m.hp = m.max_hp
        # Scale minis to be visible but smaller than main boss
        m.radius = 60 
        m.damage_mult = 2
        m.is_boss = False
    ...
    def draw(self, surface):
        self.void_rotation += 0.02
        ticks = pygame.time.get_ticks()
        pulse = (math.sin(ticks * 0.005) + 1) / 2
        
        # Glitch effect for position
        draw_pos = self.pos.copy()
        if self.glitch_timer > 0 or (self.phase == 2 and random.random() < 0.1):
            draw_pos.x += random.uniform(-20, 20)
            draw_pos.y += random.uniform(-20, 20)

        # Draw massive void aura
        aura_radius = self.radius * 1.5
        s = pygame.Surface((aura_radius * 2, aura_radius * 2), pygame.SRCALPHA)
        for i in range(8):
            r = self.radius * (1.0 + i * 0.1) * (1 + pulse * 0.05)
            alpha = int(100 - i * 12)
            color = (60, 0, 120, alpha) if self.phase == 1 else (120, 0, 60, alpha)
            pygame.draw.circle(s, color, (aura_radius, aura_radius), r)
        
        # Rotating void rings
        for i in range(4):
            angle = self.void_rotation * (1 + i * 0.5)
            r = self.radius * (0.8 + i * 0.15)
            rect = pygame.Rect(aura_radius - r, aura_radius - r, r * 2, r * 2)
            ring_color = (150, 50, 255, 180) if self.phase == 1 else (255, 50, 100, 180)
            pygame.draw.arc(s, ring_color, rect, angle, angle + 2.5, 8)
            
        surface.blit(s, (int(draw_pos.x - aura_radius), int(draw_pos.y - aura_radius)))
        ...
        # Main Body (Black Hole)
        body_color = (5, 5, 10) if self.phase == 1 else (15, 0, 5)
        pygame.draw.circle(surface, body_color, (int(draw_pos.x), int(draw_pos.y)), self.radius)
        border_color = (150, 0, 255) if self.phase == 1 else (255, 0, 100)
        pygame.draw.circle(surface, border_color, (int(draw_pos.x), int(draw_pos.y)), self.radius, 6)
        
        # Void cracks / energy lines
        for i in range(12):
            angle = (ticks * 0.0015 + i * (math.pi / 6))
            crack_color = (100, 100, 255, 200) if self.phase == 1 else (255, 100, 100, 200)
            p1 = draw_pos + pygame.math.Vector2(math.cos(angle), math.sin(angle)) * (self.radius * 0.3)
            p2 = draw_pos + pygame.math.Vector2(math.cos(angle), math.sin(angle)) * (self.radius * 0.95)
            pygame.draw.line(surface, crack_color, p1, p2, 4)

        # Central Core
        core_pulse = (math.sin(ticks * 0.01) + 1) / 2
        core_color = (100, 100, 255) if self.phase == 1 else (255, 50, 50)
        core_radius = 50 + core_pulse * 15
        
        # Core glow
        core_s = pygame.Surface((core_radius * 4, core_radius * 4), pygame.SRCALPHA)
        pygame.draw.circle(core_s, (*core_color, 120), (core_radius * 2, core_radius * 2), core_radius * 2)
        ...
        pygame.draw.circle(surface, WHITE, (int(draw_pos.x), int(draw_pos.y)), core_radius * 0.5)
        
        # Flash effect if taking damage
        if getattr(self, 'flash_timer', 0) > 0:
            pygame.draw.circle(surface, self.flash_color, (int(draw_pos.x), int(draw_pos.y)), self.radius, 15)
